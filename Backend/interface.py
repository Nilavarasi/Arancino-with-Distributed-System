...
class ContractInterface(object):
    ...
    print("default_vars_path", default_vars_path)
    ...
    def compile_source_files(self):
        ...
        for contract in os.listdir(self.contract_directory):
            deployment_list.append(os.path.join(
                self.contract_directory, contract))
    ...
    def deploy_contract(self, deployment_params=None):
        ...
        print("all_compiled_contracts")
        print(self.all_compiled_contracts)
        for compiled_contract_key in self.all_compiled_contracts.keys():
            print("contract_to_deploy", self.contract_to_deploy)
            print("compiled_contract_key", compiled_contract_key)
            if self.contract_to_deploy in compiled_contract_key:
                ...
                deployment_estimate = deployment.constructor(
                ).estimateGas(transaction=deployment_params)
                print("deployment_estimate", deployment_estimate)
    ...
    def send(self, function_, *tx_args, event=None, tx_params=None):
        ...
        if gas_estimate < self.max_tx_gas:
            ...
            print(
                f"Sending transaction to {function_} with {tx_args} as arguments.\n")
